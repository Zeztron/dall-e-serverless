...
const Images = () => {
  ...
  return (
    <div className='mb-10'>
      ...
      <div className='grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 px-10'>
          ...
          >
            <div className='absolute flex justify-center items-center w-full h-full bg-white opacity-0 hover:opacity-80 transition-opacity duration-200 z-10'>
              <p className='text-center font-light text-lg p-5'>
                {image.name.trim().split('_')[0]}
              </p>
            </div>
            <Image
              src={image.url}
              alt={image.name.trim().split('_')[0]}
              width={800}
              height={800}
